...
func foo1() {
	...
	q := fmt.Sprintf("SELECT * FROM foo where name = '%s'", os.Args[1]); rows, err := db.Query(q)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func foo3() {
	...
	q := fmt.Sprintf("select * from foo where name = '%s'", os.Args[1]); rows, err := db.QueryContext(context.Background(), q)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func foo4() {
	...
	defer tx.Rollback()
	q := fmt.Sprintf("select * from foo where name = '%s'", os.Args[1]); rows, err := tx.QueryContext(context.Background(), q)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
